...
def contain_lengthening_word(sentence):
    ...
    if re.search(pattern, sentence) and not check_except_string(sentence):
        return True
    else:
        return False
...
def process_upper_case_text(text):
    text = text.split()
    upper_case_word = []
    for word in text:
        if word.isupper() and len(word) > 2:
            upper_case_word.append(word)
    return len(upper_case_word), upper_case_word
